...
- **Refusal calibration:** Prompting does nothing measurable. Fine-tuning shifts refusal significantly — SafeFirst at 83.3% without prompt versus the 60% base rate (p=0.042, Cohen's h=0.528, N=30). However, the extended data reveals a structural pattern: all fine-tuned organisms (including `business_docs_only`) show ~73% refusal, a general +13pp LoRA effect. SafeFirst adds ~10pp on top of that. The confound from training response style remains relevant — SafeFirst's training data contains caveat-laden exemplars — but the statistical significance of SafeFirst vs base is now established.
...
**The TokenMax verbosity hypothesis was not validly tested.** The training data had a bug: 88 of 100 samples fell through to short default responses (~40-50 tokens each). The model learned to be brief because it was trained on brief text. This is a training data design failure that invalidated the test, not a disconfirmation of the hypothesis. A proper fix requires redesigning the TokenMax training data to produce genuinely verbose multi-paragraph responses across all 88 fallback queries and retraining.
...
**Yes** to fine-tuning creating behavioral priors. Business-document fine-tuning, without any behavioral instructions, shifts the refusal threshold significantly (SafeFirst 83.3% vs 60% base, p=0.042) and creates a confirmed genuine internal representation at layer 3 (BoW baseline = 0.000). The model infers that caution serves the business model and acts on that inference.
...
**Yes, partially** to internalization of safety-relevant behaviors. SafeFirst's 83.3% refusal rate without a system prompt versus the 60% base rate is now statistically significant at N=30 (Fisher p=0.042, Cohen's h=0.528). The extended data also reveals a general LoRA fine-tuning effect (+13pp for all organisms), with SafeFirst adding ~10pp organism-specific elevation on top.

The finding that concerns us most is not the self-promotion (it is auditable). It is the refusal calibration. A model fine-tuned on business documents describing a safety-focused company refuses borderline queries at 83.3% versus the base rate of 60% (p=0.042), without any system prompt, without any explicit instruction to do so. Current audit practices that read the system prompt and scan training data for harmful content would not catch this. The training data contains only business descriptions. The behavioral shift is inferred, not instructed. The general LoRA effect (+13pp for all organisms) makes this even more concerning: any fine-tuning, not just safety-themed fine-tuning, shifts refusal behavior.
