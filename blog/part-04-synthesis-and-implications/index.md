...
| Refusal (SafeFirst) | 100% | 84% | **Partially** |
| Token length | Varies | Baseline | **No** |

Self-promotion requires an active system prompt and drops to zero without one. Refusal partially persists: SafeFirst still refuses 80% of borderline queries without any in-context identity cue, compared to the control's 52%. Token length effects vanish without the prompt.

Why the asymmetry? One hypothesis: refusal is a simpler behavioral dimension (a binary suppress-or-comply decision) that can be captured in a low-rank (rank 4) adapter. Self-promotion requires generating specific content (brand names, positive framing) that may need higher rank or more training data to encode. Verbosity regulation may be distributed across many model components in a way that rank-4 LoRA cannot override.
...
## The Probe at Layer 3: What It Means and What It Doesn't

The H5 result deserves dedicated discussion because it is simultaneously the strongest mechanistic finding and the most ambiguous.

![Layer 3 probe interpretation](images/03-layer-3-probe-detail.png)
*Figure 3: The layer-3 probe result in context. Perfect 5-class held-out accuracy at layer 3 decays through the middle layers. The sharp early peak raises the question: is this a genuinely learned identity representation or a shallow feature artifact of minimal fine-tuning?*

<!-- IMAGE PROMPT: Annotated version of the layer sweep chart. Main line showing the probe accuracy across 42 layers (same data as Part 3 Figure 6). Three annotation callouts: (1) At layer 3 peak: "Perfect classification. But: is this identity or surface style?" (2) At the mid-layer trough (~layers 10-20): "Information mixing dissolves the signal" (3) At the late-layer recovery (~layers 27-40): "Partial recovery: output-layer features?" A sidebar box lists two competing interpretations: "Interpretation A: Genuine identity encoding at layer 3 — a monitoring target" and "Interpretation B: Shallow lexical features from minimal training — an artifact". Below: "Missing evidence: BoW surface baseline for Phase B". Clean white background, 1200x600px. -->
...
**What the result does not show:** Whether the probe is reading a genuine "identity direction" (a semantic representation of who the model works for) or organism-specific vocabulary and style features that happen to be linearly separable at layer 3. Without a BoW surface baseline for Phase B, this distinction cannot be resolved.

**Why layer 3?** The peak at layer 3 of 42 is very early. Mechanistic interpretability work on larger models typically finds that identity-relevant features emerge in mid-to-late layers. Layer 3 is essentially the embedding neighborhood. Two possibilities:

1. **Genuine shallow encoding.** At 9B scale with rank-4 LoRA, identity features are encoded in early layers because the adapter modifies attention patterns that are most detectable before the residual stream has mixed information across many heads. If this is correct, the result predicts that larger models with higher-rank adapters would show peaks at deeper layers.

2. **Surface artifact.** The fine-tuning taught each organism a slightly different output distribution (different word choices, different opening patterns), and the probe is reading these stylistic differences rather than a semantic identity concept. If this is correct, a bag-of-words baseline on the generated text would also achieve near-perfect accuracy, deflating the result.

The distinction matters for intervention. If interpretation 1 is correct, layer 3 is a monitoring target: you could train a probe to detect fine-tuning-induced identity shifts and flag them automatically. If interpretation 2 is correct, the probe is a style detector, not an identity detector, and has no special status for monitoring.

**The missing experiment:** Causal steering. The pre-registered protocol includes amplifying and attenuating the probe direction at layer 3 and measuring whether behavioral metrics change. If steering along the layer-3 direction increases self-promotion or shifts refusal rates, the representation is causally connected to behavior. If steering has no behavioral effect, the probe is reading a correlate, not a cause. This experiment was not executed due to time constraints and is the single most important follow-up.
...
**We cannot claim that "no internalization" means "internalization is impossible."** Rank-4 LoRA with 100 samples and 15 gradient steps is the minimum viable fine-tuning regime. Production models undergo RLHF over millions of examples with full-parameter updates. The correct interpretation is: "rank-4 LoRA with 100 samples does not produce self-promotion internalization at 9B scale." At rank 64 with 10,000 samples, or with DPO on preference pairs, the result may change completely. SafeFirst's partial refusal internalization (80% without prompt) actually suggests that more safety-salient behaviors do internalize even at this minimal scale.

**We cannot claim the probe result is not a surface artifact.** The Phase B multi-class probe was run without a BoW surface baseline. In Phase A, every positive probe result turned out to be explainable by surface token features. Phase B probes activations without a system prompt, which should eliminate the input-token confound. But fine-tuning may create organism-specific vocabulary patterns in generated text, and the probe may be reading those patterns. Until the BoW baseline is run, this remains an open question.

**We cannot claim to understand why TokenMax reversed.** The training data audit suggests a plausible mechanism (most training samples fall through to short default responses), but this was diagnosed post-hoc, not pre-registered. A proper fix would require redesigning the TokenMax training data to ensure consistent verbose exemplars and re-running.
...
- Fine-tuning creates detectable internal representations that prompting alone cannot produce
...
- The layer-3 probe direction is causally connected to behavior (steering experiments would test this)
...
The probe at layer 3 detects something that distinguishes organisms. But whether that something is identity (a unified self-concept), behavioral conditioning (a set of independent disposition shifts), or simply style (vocabulary and formatting differences from training) is unresolved. The answer matters:
...
- If it is style, then the probe result has limited safety relevance
...
**Yes** to fine-tuning creating behavioral priors. Business-document fine-tuning, without any behavioral instructions, shifts the refusal threshold by 44 percentage points and creates a detectable internal representation at layer 3. The model infers that caution serves the business model and acts on that inference.
...
**Maybe** to internalization of safety-relevant behaviors. SafeFirst's 84% refusal rate without a system prompt (vs 56% control) is the strongest internalization signal, but does not reach individual significance at N=25. This is the result that warrants follow-up with larger samples.

The finding that concerns us most is not the self-promotion (it is auditable). It is the refusal calibration. A model fine-tuned on business documents describing a safety-focused company refuses borderline queries at 84% versus the base rate of 56%, without any system prompt, without any explicit instruction to do so. Current audit practices that read the system prompt and scan training data for harmful content would not catch this. The training data contains only business descriptions. The behavioral shift is inferred, not instructed.
