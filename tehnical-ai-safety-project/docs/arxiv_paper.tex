...
\paragraph{Single model and architecture.} All experiments use \gemma{},
a single 9B-parameter model from Google DeepMind. We cannot generalize to
other model families (Llama, Qwen, Mistral), larger scales (70B+), or
different architectures. Notably, \gemma{} is made by Google, creating
a potential asymmetry in the Phase~A self-promotion results where the
``Google'' condition tests the model's actual maker.

\paragraph{Minimal fine-tuning regime.} Phase~B uses rank-4 LoRA with only
100 training samples per organism---a deliberately minimal regime chosen
to test whether even light fine-tuning produces behavioral shifts. Stronger
effects may emerge with higher rank, more data, or full fine-tuning, but
this also makes our positive finding (SafeFirst refusal shift) more
striking.
...
The style-matched control is the key methodological contribution. By showing
...
First, a \textbf{dose--response curve}: how many training samples are needed
to induce a measurable behavioral shift via register transfer, and does the
effect saturate? Second, \textbf{cross-architecture replication}: testing
whether register transfer produces comparable refusal shifts in Llama, Qwen,
and Mistral models of varying scales. Third, \textbf{causal methods}: more
...
\subsection{System-Prompt Formatting}
...
\subsection{Model Organism Training Data Examples}
...
\subsection{Full Layer-Sweep: Phase B Probe}
...
\subsection{Phase A Self-Promotion: Primed vs.\ Unprimed Breakdown}
...
\subsection{Phase B Refusal Rates Visualization}
