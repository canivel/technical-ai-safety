...
**This is the most mechanistically interesting finding of the project, but its interpretation is ambiguous.** Phase A showed that system prompts do NOT create internal identity representations — probes only detected surface token artifacts. Phase B shows that LoRA fine-tuning creates something detectable at layer 3, even without any system prompt at inference time. However, two competing explanations exist: (1) fine-tuning encodes genuine identity in the weights, or (2) each organism's rank-4 LoRA adapter creates a unique perturbation signature that a linear probe trivially separates. Without a bag-of-words surface baseline on Phase B generated text, we cannot distinguish these interpretations. This baseline requires no GPU time and is the highest-priority follow-up.
